...
export interface QqInfoResult {
  /** QQ 头像地址(后端 /qq-info 已完成降级,恒非空;仅后端 avatar-api 也未配置时为空串) */
  avatarUrl: string;
 ...
 * <p>
 * 头像:后端 /qq-info 优先调用 qq-api 解析,失败时已按 avatar-api 以 QQ 号为种子
 * 降级拼接,同一 QQ 号每次生成的头像一致;仅后端 avatar-api 也未配置时为空,
 * 此时回退本地兜底图。
